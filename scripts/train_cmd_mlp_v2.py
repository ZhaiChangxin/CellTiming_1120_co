...
def cmd_loss(x: torch.Tensor, y: torch.Tensor, n_moments: int = 5) -> torch.Tensor:
    ...
    计算 1~n 阶中心矩的 L2 差之和（做了简单的归一化，避免量级过大）
    """
    ...
    loss = torch.norm(mx - my, p=2) / (D ** 0.5)
    ...
    for k in range(2, n_moments + 1):
        ...
        mxk = cx.mean(dim=0)
        myk = cy.mean(dim=0)
        loss = loss + torch.norm(mxk - myk, p=2) / (D ** 0.5)
...
def main():
    ...
    # 迁移相关超参
    ap.add_argument("--lambda_cmd_max", type=float, default=0.1,
                    help="CMD 的最终最大权重（会从 0 线性涨到这个值）")
    ap.add_argument("--lambda_tgt", type=float, default=2.0,
                    help="目标域有标签监督的权重")
    ap.add_argument("--warmup_epochs", type=int, default=20,
                    help="前多少个 epoch 不加 CMD，只做监督学习")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        # 计算当前 epoch 的 CMD 权重（warmup + 线性ramp）
        if ep <= args.warmup_epochs:
            lambda_cmd = 0.0
        else:
            # 从 warmup_epochs+1 开始线性涨到 lambda_cmd_max
            t = (ep - args.warmup_epochs) / max(args.epochs - args.warmup_epochs, 1)
            t = max(0.0, min(1.0, t))
            lambda_cmd = args.lambda_cmd_max * t

        print(f"[Epoch {ep}] lambda_cmd = {lambda_cmd:.4f}")
        ...
        for _ in range(len(loader_src)):
            ...
            loss = loss_src + args.lambda_tgt * loss_tgt + lambda_cmd * loss_cmd
        ...
        if rmse_t < best_rmse_tgt:
            ...
            best_state = {
                ...
                "hid_dim": args.hid,
                "depth": args.depth,
                "lambda_cmd_max": args.lambda_cmd_max,
